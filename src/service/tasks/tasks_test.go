...
import (
	...
	"TravelService/src/service/tasks"
	"bytes"
	"encoding/json"
...
type tasksCRUDTestCase struct {
	...
	url              string
	want             int
	mockedGetTask    model.Task
	mockedGetTasks   []model.Task
	mockedDeleteTask uuid.UUID
	mockedCreateTask model.Task
...
func TestGetTasks(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			model.MockedGetTasks(tc.mockedGetTasks, tc.mockedTasksError)
...
func TestGetTasksByID(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			model.MockedGetTask(tc.mockedGetTask, tc.mockedTasksError)
			rec := httptest.NewRecorder()
			req, _ := http.NewRequest(http.MethodGet, tc.url, nil)
...
func TestDeleteTasks(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			model.MockedDeleteTask(tc.mockedDeleteTask, tc.mockedTasksError)
			rec := httptest.NewRecorder()
...
func TestCreateTasks(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			model.MockedCreateTask(tc.mockedCreateTask, tc.mockedTasksError)
			rec := httptest.NewRecorder()
...
func TestGetUserTasks(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			rec := httptest.NewRecorder()
			req, _ := http.NewRequest(http.MethodGet, tc.url, nil)
			model.MockedGetUserTasks(tc.mockedGetTasks, tc.mockedTasksError)
			router.ServeHTTP(rec, req)
			if rec.Code != tc.want {
				t.Errorf("Expected: %d , got %d", tc.want, rec.Code)
			}
